...
func newSwaggerParameter(name, in string, Value *Value) (*swagger.Parameter, error) {
	dataType, ok := dataTypes[Value.Type]
	if !ok {
		err := errors.New(Value.Type + " is not supported")
		return nil, &parameterError{name, err}
	}
	min, err := Value.getMinimum()
	if err != nil {
		return nil, err
	}
	max, err := Value.getMaximum()
	if err != nil {
		return nil, err
	}
	...
	minLen, err := Value.getMinLen()
	if err != nil {
		return nil, err
	}
	maxLen, err := Value.getMaxLen()
	if err != nil {
		return nil, err
	}
	defaultValue, err := Value.getDefalut()
	if err != nil {
		return nil, err
	}
