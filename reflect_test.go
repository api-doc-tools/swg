...
func TestGetStructFieldName(t *testing.T) {
	...
	for i := 0; i < structType.NumField(); i++ {
		...
		assert.Nil(err)
		assert.Equal(fieldName, fieldNameList[i])
	}
}
...
func TestGetObjName(t *testing.T) {
	...
	for _, v := range ValidList {
		...
		assert.Nil(err)
		assert.Equal(v.StructNameToIdentify, name)
	}

	InvalidList := InvalidListForTestGetObjName()
	for _, v := range InvalidList {
		_, err := getStructReflectType(v.Object)
		assert.NotNil(err)
	}
}
...
func TestGetStructDocMap(t *testing.T) {
	...
	creater := StructDocCreater{}
	_, err := creater.GetStructDocMap(S4{})
	assert.Nil(err)
}
